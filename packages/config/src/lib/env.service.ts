...
import { EnvDefault, EnvKey } from '@vnodes/env';
...
export class EnvService {
  ...
  get ALLOWED_ORIGINS() {
    return process.env[EnvKey.ALLOWED_ORIGINS];
  }

  get NODE_ENV() {
    return process.env[EnvKey.NODE_ENV];
  }
  get IS_PROD() {
    return this.NODE_ENV && /prod/gi.test(this.NODE_ENV);
  }

  get IS_DEV() {
    return this.NODE_ENV && /dev/gi.test(this.NODE_ENV);
  }
  ...
  get DB_POOL_IDLE_TIMEOUT_MILLIS() {
    return Number(
      this.get(EnvKey.DB_POOL_IDLE_TIMEOUT_MILLIS, EnvDefault.DB_POOL_IDLE_TIMEOUT_MILLIS),
    );
  }
  get DB_POOL_MAX_USES() {
    return Number(this.get(EnvKey.DB_POOL_MAX_USES, EnvDefault.DB_POOL_MAX_USES));
  }
  get DB_POOL_STATEMENT_TIMEOUT() {
    return Number(this.get(EnvKey.DB_POOL_STATEMENT_TIMEOUT, EnvDefault.DB_POOL_STATEMENT_TIMEOUT));
  }
  get DB_POOL_QUERY_TIMEOUT() {
    return Number(this.get(EnvKey.DB_POOL_QUERY_TIMEOUT, EnvDefault.DB_POOL_QUERY_TIMEOUT));
  }
  get DB_POOL_LOCK_TIMEOUT() {
    return Number(this.get(EnvKey.DB_POOL_LOCK_TIMEOUT, EnvDefault.DB_POOL_LOCK_TIMEOUT));
  }
}
