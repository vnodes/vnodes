...
import type { UserRoleCreateDto, UserRoleQueryDto, UserRoleUpdateDto } from './dtos/index.js';
import { UserRoleQueryService } from './user-role-query.service.js';

@Injectable()
export class UserRoleService implements ResourceOperations {
    constructor(
        @InjectDelegate('userRole') protected readonly repo: P.Prisma.UserRoleDelegate,
        @Inject(UserRoleQueryService) protected readonly queryService: UserRoleQueryService,
    ) {}

    async validateUniques(data: Partial<P.Prisma.UserRoleModel>, id?: number) {
        const uniqueFields: P.Prisma.UserRoleScalarFieldEnum[] = [];
    ...
    async find(query: UserRoleQueryDto) {
    ...
    async findByIdOrThrow(id: number) {
        try {
            ...
            throw new NotFoundException('A UserRole with the id is not found');
        }
    }
